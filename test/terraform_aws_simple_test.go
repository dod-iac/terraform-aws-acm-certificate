...
func TestTerraformSimpleExample(t *testing.T) {
	...
	testName := fmt.Sprintf("terratest-acm-certificate-simple-%s", strings.ToLower(random.UniqueId()))
	...
	outputDomainValidationOptions := terraform.Output(t, terraformOptions, "domain_validation_options")
